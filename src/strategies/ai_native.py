...
class AINativeDecisionMaker:
    ...
    async def _pro_deep_analyze(self, inp: AIInput) -> Optional[dict]:
        ...
        prompt = (
            ...
            + (f"5m RSI={inp.ind_5m.get('rsi',50):.0f} MA10={inp.ind_5m.get('ma10',0):.2f} MA30={inp.ind_5m.get('ma30',0):.2f} "
               f"MACD={inp.ind_5m.get('macd',0):.4f} ATR={inp.ind_5m.get('atr',0):.2f} "
            ...
            + (f"15m RSI={inp.ind_15m.get('rsi',50):.0f} MA10={inp.ind_15m.get('ma10',0):.2f} MA30={inp.ind_15m.get('ma30',0):.2f} "
               ...
               f"VWAP={inp.ind_15m.get('vwap',0):.2f} 量比={inp.ind_15m.get('volume_ratio',1):.1f} BB={inp.ind_15m.get('bb_position',0.5):.2f}\n" if inp.ind_15m else "")
    ...
    async def manage_position(self, inp: AIInput) -> Optional[Signal]:
        ...
        prompt = (
            ...
            + hist_str
            + "\n日内持仓管理原则(铁律):\n"
            "1. 移动止损锁利: 浮盈≥1倍风险时止损移到保本; 浮盈≥2倍风险时止损移到1倍风险处;\n"
            "   趋势仍强(ADX高+动量正)时让利润奔跑, 绝不在浮盈微小(不足0.5%)时手动平仓\n"
            "2. 最小盈利保护: 手续费每轮约0.06%-0.12%, 浮盈小于0.5%时不平仓(还不够付手续费);\n"
            "   只有浮盈≥1%才考虑落袋\n"
            "3. 回撤保护: 浮盈从最高点回吐超过50%时收紧止损至保本或1R, 但除非趋势明确反转否则不平仓\n"
            "4. 时间因素: 持仓时长本身不是平仓理由; 盘后流动性差≠必须平仓, 只在\n"
            "   (a)趋势反转信号明确 (b)浮盈回吐超50% (c)浮盈≥目标位 三者之一时才平仓\n"
            "5. 风险平衡: 止损距离必须匹配当前ATR/波动率, 太紧易被扫, 太宽回撤大\n"
            "6. 只在趋势+动量+量能共振确认反转时果断平仓, 不因小幅波动恐慌离场\n\n"
            "输出JSON:\n"
