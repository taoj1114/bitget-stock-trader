...
class AINativeDecisionMaker:
    ...
    async def _pro_deep_analyze(self, inp: AIInput) -> Optional[dict]:
        ...
        prompt = (
            ...
            "          weekend周末休市必HOLD。\n"
            "日内交易原则(快进快出为核心):\n"
            "  1. 目标止盈: 0.8%-2% 的小目标, 到目标果断离场, 不贪大(日内吃波段, 不赌趋势)\n"
            "  2. 止损铁律(防止被噪音扫掉!):\n"
            "     - 止损距离必须 ≥ 1.5×5m ATR, 通常 1%-2% (5m正常波动就有0.5-0.7%, 放更近必被扫!)\n"
            "     - 止损放在结构位(近2小时高低点)外侧 + 0.5×ATR缓冲——先用ATR过滤噪音, 再找结构位锚定\n"
            "     - 宁可止损距离稍大(1.5%-2%)也不要设0.5%以下的紧止损——紧止损必被日内噪音扫掉\n"
            "     - 止损与止盈至少1:1, 目标1:1.5 (靠'止损不被扫+止盈到位'双赢)\n"
            "  3. 持仓时间: 30分钟-3小时, 到点不到目标也离场(时间止损, 避免隔夜)\n"
            "  4. 浮盈≥0.5%后止损移到保本, ≥1%后移到浮盈一半(移动止损锁利)\n"
            "  5. 不追已大幅透支的行情; 入场时若当日已涨/跌超5%, 只做反转不追高\n"
            "  6. 盈利保护优先: 浮盈达到0.8%就果断落袋一半, 剩余移保本——不让到手的利润回吐\n"
        )
    ...
    async def manage_position(self, inp: AIInput) -> Optional[Signal]:
        ...
        prompt = (
            ...
            "1. 小目标落袋: 浮盈达0.8%-2%即果断止盈离场(日内吃波段, 不贪大, 到目标就走; 0.5%以下太薄, 扣除0.12%手续费后意义不大)\n"
