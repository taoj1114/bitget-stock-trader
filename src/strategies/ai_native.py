...
class AINativeDecisionMaker:
    ...
    async def _pro_deep_analyze(self, inp: AIInput) -> Optional[dict]:
        ...
        prompt = (
            ...
            "      - 当日已涨>15% → 只做回踩, 禁止追涨\n"
            "      - 1H RSI>85 → 极度超买, 禁止追涨\n"
            "      - 价格距近2小时高点<0.2% → 已贴顶, 无空间, 禁止追涨\n"
            "      - 止损无法放在近2小时低点下方(结构位) → 禁止追涨\n"
            "      → 追涨必须: 止损在结构位下 + 盈亏比≥1\n"
            "  多3 大盘共振: 大盘(SPY/QQQ)强 + 个股跟涨→顺势买\n"
            "  多4 禁止: 大盘弱(SPY下跌)时逆势抄底个股; 当日已涨>15%追高\n"
            "═══ 做空 (SELL) ═══\n"
            ...
            "      逼空风险——止损必须更紧或仓位更小\n"
            "═══ 通用 ═══\n"
            "  场景判断: 顺势趋势(ADX>20)用多1/空1; 横盘(ADX<20)等突破; 反转初期(1H未转5m已转)突破确认吃第一波\n"
            "  位置判断: 价格远离MA30(>1.5×ATR)+BB贴边(>0.9/<0.1)=透支→等回踩/反抽, 不追\n"
            "  未透支时顺势入场是合理的, 不要过度等待完美回踩\n"
            "  长上影/长下影+RSI极端(>85/<15)警惕反转吞没→HOLD等确认\n"
            "  量价背离(新高但量比<0.5)=假突破→不入场\n"
            "  当日已涨/跌>15%视为极端行情——只做回踩/反抽, 但8%-15%之间属正常强势, 顺势可追\n"
