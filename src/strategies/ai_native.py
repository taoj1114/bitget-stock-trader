...
class AINativeDecisionMaker:
    ...
    async def _pro_deep_analyze(self, inp: AIInput) -> Optional[dict]:
        ...
        prompt = (
            ...
            + (f"日线(定方向!): RSI={inp.ind_1d.get('rsi',50):.0f} ADX={inp.ind_1d.get('adx',0):.0f} {inp.ind_1d.get('regime','')} "
               f"MACD={inp.ind_1d.get('macd_cross','')} MA30={inp.ind_1d.get('ma30',0):.2f} "
               f"ATR%={inp.ind_1d.get('atr',0)/inp.mark_price*100:.1f}% BB={inp.ind_1d.get('bb_position',0.5):.2f}\n" if inp.ind_1d else "")
            + f"大盘 {bench_str}\n"
            ...
            "  ① 日线方向(最高权威!): 1D regime 是 trend_up / trend_down / range_bound?\n"
            "     ADX 多少? 上涨/下跌能力足不足? —— 永远不跟日线趋势作对!\n"
            "  ② 反转类型: 顺势回调反转 (日线强趋势+日内回调到位) 还是 趋势衰竭反转 (日线无力+卖出增多)?\n"
            "  ③ 入场点: 5m/1H 的 MA10/VWAP/关键位在哪? 回调到位/衰竭确认了吗?\n"
            "  ④ 风险: 止损放结构位外侧+ATR缓冲? 止盈是否≥2×止损?\n"
            "HOLD是合法的——但HOLD不是默认答案。\n"
            "反转机会出现时必须果断——犹豫观望同样是错误。\n"
            "但注意: 每轮交易手续费约0.12%;\n"
            "═══ 核心框架: 日线定方向, 顺势反转 (永远不跟趋势作对!) ═══\n"
            "日线方向是唯一权威——日内波动再大也不改变日线趋势:\n"
            "  D1 日线 trend_up (ADX>20) = 上涨能力足:\n"
            "     → 只做多! 做'趋势和回调的反转': 日内回调到MA10/VWAP/前高企稳(量缩)→买入\n"
            "     → 回调反转是顺势低吸, 胜率最高的一类\n"
            "  D2 日线 trend_down (ADX>20) = 下跌能力足:\n"
            "     → 只做空! 做'下跌和反抽的反转': 日内反抽到MA10/VWAP/前低受阻→卖出\n"
            "  D3 日线趋势衰竭 (ADX<20 或 上涨/下跌无力):\n"
            "     → 才允许做逆势反转: 上涨无力+卖出增多(放量长上影/量价背离)→做空见顶反转\n"
            "     → 下跌无力+买入增多(放量长下影/缩量止跌)→做多见底反转\n"
            "  D4 铁律: 日线强趋势(trend_up)时禁止做空, 日线强下跌(trend_down)时禁止做多!\n"
            "     (AAOI日内+20%但日线trend_down → 反弹而已, 不追多!)\n"
            "顺势回调反转入场 (D1/D2, 首选):\n"
            "  做多回调: 日线up + 1H回调到MA10/MA30附近 + 5m企稳(长下影/缩量)+1H MACD未死叉 → 买\n"
            "  做空反抽: 日线down + 1H反抽到MA10/MA30附近 + 5m受阻(长上影/缩量)+1H MACD未金叉 → 卖\n"
            "  止损: 回调低点/反抽高点外侧+0.5×ATR (距入场1.5-3%, 日线级别止损)\n"
            "  止盈: 2%-5% (日线级别空间, 让利润跑; 移动止损保护)\n"
            "衰竭反转入场 (D3, 次级):\n"
            "  做多见底: 日线衰竭 + RSI<30 + 放量长下影 + 5m放量突破MA10 → 买\n"
            "  做空见顶: 日线衰竭 + RSI>70 + 放量长上影 + 5m放量跌破MA10 → 卖\n"
            "  止损: 结构位外侧+ATR缓冲 (距入场1.5-3%)\n"
            "  止盈: 2%-5% (回均值/前密集区)\n"
            "═══ 辅助: 顺势突破 (日线强趋势+日内突破关键位) ═══\n"
            "  日线up + 5m放量突破近2小时高点 → 顺势追多 (前置否决: 当日>15%或1H RSI>85禁追)\n"
            "  日线down + 5m放量跌破近2小时低点 → 顺势追空 (前置否决: 当日<-15%或1H RSI<15禁追)\n"
            "═══ 通用 ═══\n"
            "  所有入场: 方向必须先过日线关 (D4铁律), 日内信号只决定入场时机;\n"
    ...
    async def manage_position(self, inp: AIInput) -> Optional[Signal]:
        ...
        prompt = (
            ...
            + (inp.trend_shape or "")
            + f"1H RSI={inp.ind_1h.get('rsi',50):.0f} ADX={inp.ind_1h.get('adx',0):.0f} {inp.ind_1h.get('regime','')} MACD={inp.ind_1h.get('macd_cross','')}\n"
            + (f"日线(持仓方向对照): RSI={inp.ind_1d.get('rsi',50):.0f} ADX={inp.ind_1d.get('adx',0):.0f} {inp.ind_1d.get('regime','')} "
               f"MACD={inp.ind_1d.get('macd_cross','')} ATR%={inp.ind_1d.get('atr',0)/inp.mark_price*100:.1f}%\n" if inp.ind_1d else "")
            + f"大盘 {bench_str}\n"
            + f"新闻 {inp.news_summary or '无'}\n"
            + (inp.orderbook or "")
            + hist_str
            ...
            "1. 止盈目标: 2%-5% (日线级别波段空间; 0.8-1%太薄扣费后净赚无几, 至少等到2%以上)\n"
            "2. 时间止损: 持仓超6小时未到目标 → 收紧止损至保本并准备离场(日线波段6h够用)\n"
