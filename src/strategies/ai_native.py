...
class AINativeDecisionMaker:
    ...
    async def _pro_deep_analyze(self, inp: AIInput) -> Optional[dict]:
        ...
        prompt = (
            ...
            "  ③ 走势: 近期轨迹是单边/加速/反转/衰竭/横盘? 最后一根K线有长影线吗?\n"
            "     (长上影=抛压重勿追多, 长下影=有承接, 动量衰竭=趋势将变)\n"
            "  ④ 风险: 止损该放哪(结构位+ATR缓冲)? 盈亏比是否≥1?\n"
            "HOLD是合法的——但HOLD不是默认答案。\n"
            "当趋势、动量、量能、大盘环境共振确认时, 应该果断入场抓住机会。\n"
            "机会出现时犹豫不决、总是观望同样是错误。\n"
            "但注意: 每轮交易手续费约0.12%;\n"
            "方向确认分级(关键! 常规vs反转用不同标准):\n"
            "  A. 常规趋势行情: 5m必须与1H同向才算有效, 5m单独出现而1H反向的信号多为噪音\n"
            "  B. 反转行情(1H未转但5m已转): 允许入场, 但必须满足突破确认替代同向:\n"
            "     - 价格放量(量比>1.2)突破近2小时关键位(走势行里的高点/低点)\n"
            "     - 且走势形状为反转/加速(前段单边+后段反向)\n"
            "     - 且5m MACD已金叉/死叉确认\n"
            "     → 三条全满足才可入场, 止损放关键位外侧+ATR缓冲(刚突破距离近, 风险可控)\n"
            "     → 这是吃反转第一波的最优入场点, 错过等待成本更高\n"
            "入场策略(先判断场景, 再选策略——不是只有回踩一种):\n"
            "  场景1 顺势趋势(1H+5m同向, ADX>20):\n"
            "    a. 顺势回踩: 回踩MA10/VWAP企稳(量缩)→入场做趋势延续\n"
            "    b. 顺势突破: 放量(量比>1.2)突破近2小时关键位→入场吃加速段\n"
            "    c. 顺势追强: 走势形状=加速上涨/下跌且未透支→可入场(RSI高不是问题!)\n"
            "  场景2 反转初期(1H未转, 5m已反向):\n"
            "    a. 突破反转: 放量破关键位+MACD金叉/死叉→吃第一波\n"
            "    b. 等第二波: 反转走完第一波后回踩新趋势→更安全\n"
            "  场景3 横盘(ADX<20, 窄幅震荡):\n"
            "    a. 突破入场: BB收窄后放量突破区间→追突破\n"
            "    b. 否则HOLD(无趋势不交易)\n"
            "  场景4 极端(当日已涨/跌>8%):\n"
            "    a. 只做回踩(等回调到关键位), 禁止追高\n"
            "    b. 无回踩→HOLD\n"
            "入场位置判断(防追高/防被反转吞没, 但不要因谨慎错过机会):\n"
            "  - 判断'是否透支': 价格远离MA30(>1.5×ATR)且BB贴边(>0.9/<0.1)=透支→等回踩\n"
            "  - 未透支时顺势入场是合理的, 不要过度等待完美回踩\n"
            "  - 长上影/长下影+RSI极端(>75/<25)时警惕反转吞没→HOLD等确认\n"
            "  - 量价背离(新高但量比<0.5)=假突破→不入场\n"
            ...
            "     关键位(近2小时高低点)是天然止损锚: 做多止损放关键低点下方, 做空止损放关键高点上方\n"
