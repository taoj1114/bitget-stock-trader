...
class AINativeDecisionMaker:
    ...
    async def _pro_deep_analyze(self, inp: AIInput) -> Optional[dict]:
        ...
        prompt = (
            ...
            "入场位置铁律(防追高/防被反转吞没):\n"
            "  0. 铁律前提: RSI高本身不是禁止理由! 强势趋势股RSI>75是常态;\n"
            "     判断的是'位置'(是否透支), 不是'RSI数值'——顺势突破时RSI高完全正常\n"
            ...
            "反转行情处理(空仓时, 区分两种情况):\n"
            "  1. 1H regime尚未切换(方向未变), 但5m已先行突破关键位 → 用上方规则B突破确认入场吃第一波\n"
            "  2. 1H regime已切换(趋势刚反转) → 第一波已走完一部分, 禁止追第一波尾部;\n"
