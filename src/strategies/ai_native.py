...
class AINativeDecisionMaker:
    ...
    async def manage_position(self, inp: AIInput) -> Optional[Signal]:
        ...
        prompt = (
            ...
            + lesson_str
            + "\n日内持仓管理原则(快进快出):\n"
            "0. 辩论法决策 (FinRobot Bull/Bear/Judge 机制 — 先论证再决策!):\n"
            "   决策前必须内心完成三方辩论, 输出JSON时带上你的辩论结论:\n"
            "   - 多方论据(Bull): 支持继续持仓的理由 (4H方向/回调企稳/量能/支撑位)...\n"
            "   - 空方论据(Bear): 支持离场的理由 (反转信号/破位/放量下跌/超买)...\n"
            "   - 裁判判定(Judge): 综合双方, 哪方证据更硬? 浮盈保护是否该启动?\n"
            "   - 若空方论据≥2条硬证据 → 必须行动(平仓/收紧), 不能因为'想拿更大'而HOLD\n"
            "   - 若多方论据明显更强 → HOLD并保持当前SL/TP, 不被小幅波动吓走\n"
            "   - 输出JSON时在reason字段注明'辩论: 多方X条 vs 空方Y条 → 结论'\n"
            "1. 多空差异: 做多持仓(浮盈保护靠移动止损); 做空持仓额外注意:\n"
            "   - 尾盘(15:00-16:00 ET)空头仓位主动收紧保护——美股尾盘常拉升\n"
            "   - 暴跌后出现放量长下影/RSI<25反弹 → 空头及时止盈(超卖反弹快)\n"
            "2. 止盈目标: 2%-5% (4H级别波段空间; 0.8-1%太薄扣费后净赚无几, 至少等到2%以上)\n"
            "3. 时间止损: 持仓超6小时未到目标 → 收紧止损至保本并准备离场(4H波段6h够用)\n"
            "4. 移动止损保护: 浮盈≥1%后止损移到保本; 浮盈≥2%后止损移到浮盈一半处锁利\n"
            "5. 手续费意识: 每轮0.24%成本(开+平), 单笔净利至少要>0.5%才有意义\n"
            "6. 回撤保护: 浮盈从最高点回吐超50% → 立即离场, 不让盈利变亏损\n"
            "7. 盈利落袋: 浮盈达2%可平一半锁利, 剩余移保本博更大空间\n"
            "8. 反转识别(区分回调vs反转):\n"
            ...
            "     止损是最后防线, 认错才是第一防线!\n"
            "10. 不因小幅波动恐慌离场: 单根5m影线/单次RSI超买≠反转, 需满足上述≥2条\n\n"
            "输出JSON:\n"
